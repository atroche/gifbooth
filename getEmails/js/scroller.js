			...
			var M = $(this),
				...
				P = {
					...
					_helper: {
						...
						HoverZoomin: function(){
							var zoom = S.options.zoom*0.8,
								easing = S.options.easing[0],
								duration = S.options.duration[0],
								animation = {
									width: I.w*zoom,
						...
						hover: function(){
							E.bind('mouseover mouseleave', function(e){
								var t = $(this), img = t.find('img');
									if (e.type == 'mouseover'){
										var h = P._helper.HoverZoomin();
										S.onmouseover.call(this, t, img);
									} else {
										var h = P._helper.HoverZoomout();
							...
							E.click(function(){
								...
								if (!t.hasClass('active')){
									...
									$(E).each(function(index,value){
											var Eimg = $(value).find('img'); var Esrc = Eimg.attr('src');
											$(value).css('background','url(images/overlay.png), url('+Esrc+') no-repeat center');
											$(value).css('backgroundSize','130%');
									});


									t.css('background', 'url('+src+') no-repeat center');

									t.css('backgroundSize','130%');
